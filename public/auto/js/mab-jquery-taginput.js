...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype = {
        ...
        init: function(element) {
            ...
            //Check for Email or Regax 
            var regax = this.options.regax;
            ...
            tagInput.on('keydown typeahead:selected', function(e) {
                ...
                if(e.type === 'typeahead:selected' || (e.keyCode == KEYCODES.ENTER && $.trim(input.val()) !== '' && emailPassed(input.val())) ) {
                    // Stop the form being submitted and prevent event bubbling
                    console.log(emailPassed(input.val()));
                    e.preventDefault();
                    e.stopPropagation();
    ...
    var emailPassed =  function (regax){
        if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(regax))
        {
          return (true)
        }
