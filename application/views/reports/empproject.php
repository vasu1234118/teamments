   ...
   $admin_id = $proadmin_data['TM_ID']; $user=$this->db->get_where('users', array('id'=>$admin_id))->row(); echo $user->role  ?>
                  <label>Employewise <span class="text-danger">*</span></label>
                   ...
                   <select class="form-control select2" multiple="multiple" data-placeholder="Select the taks related"
                      style="width: 100%;" name="emp_id[]" id="task_related_to">
                   <?php  $pr=$this->db->get_where('users', array('id !='=>$admin_id))->result();  foreach($pr as $pr2){  ?>
                  <option value="<?php echo $pr2->id; ?>"><?php echo $pr2->fname ?><?php echo $pr2->name ?>  </option>
                  <?php } ?>
                </select>
                </div>
            </div> 
