...
<?php $this->load->view('include/left_nav.php'); ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>Edit<?php echo $add_title; ?>(<?php $z=$this->db->get_where('users', array('id'=>$record_info->user_id))->row();  echo $z->fname;echo $z->lname; ?>)</h1>
      <ol class="breadcrumb">
...
<!-- <?php if($record_info->status=='1' && strtotime($record_info->from_date)>strtotime(date('Y-m-d'))){  ?>
            <div class="col-md-12">
              <div class="form-check">
    <input type="checkbox" class="form-check-input" id="exampleCheck1">
    <label class="form-check-label" for="exampleCheck1">Cancel Leave</label>
  </div>
            </div>
          <?php } ?> -->
      ...
      <input type="hidden" name="edit_id" value="<?php echo md5($record_info->id); ?>" />
     <div class="text-center"><a href="<?php echo site_url($pagename); ?>" class="btn btn-default btn-md" name="submit">Cancel</a>&nbsp;&nbsp;&nbsp;&nbsp;
      <?php if($record_info->status=='1'){  if(strtotime($record_info->from_date)>strtotime(date('Y-m-d'))) { ?>

      <input type="submit" class="btn btn-warning btn-md" name="cancel" value="Cancel Leave" />&nbsp;
      <?php } }else if($record_info->status=='0'){ ?>

      <input type="submit" class="btn btn-primary btn-md" name="update" value="Update" />&nbsp;&nbsp;&nbsp;&nbsp;
      <button type="submit" class="btn btn-warning btn-md" name="save">Save</button>
       <?php }?>
     </div>
