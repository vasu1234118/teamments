...
class Requirements_model extends CI_Model{
	public $milestone_tbl='requirements';
	...
	function allRequirements(){
		$this->db->select('lt.*,u.display_name as display_name,rt.title as user_role,p.name as project_name,s.title as status_title');
		$this->db->from($this->milestone_tbl.' lt');
		$this->db->join($this->users.' u', 'lt.created_by = u.id','left');
		$this->db->join($this->project_tbl.' p', 'lt.project_id = p.id','left');
		$this->db->join($this->roles_tbl.' rt', 'rt.id = u.role','left');
		$this->db->join($this->status_tbl.' s', 'lt.status = s.id')
	...
	function allTasks($user_id){
		...
		$this->db->from($this->milestone_tbl.' ct');
	...
	function getUnreadTaskCount($userid){
		...
		$this->db->join($this->milestone_tbl." ct", "t.createtask_id=ct.id");
