...
class Idea_model extends CI_Model{
	...
	function allIdeas(){
		$this->db->select('lt.*,u.display_name as display_name,rt.title as user_role,s.title as status_title');
		$this->db->from($this->idea_tbl.' lt');
		$this->db->join($this->users.' u', 'lt.created_by = u.id','left');
		$this->db->join($this->roles_tbl.' rt', 'rt.id = u.role','left');
		$this->db->join($this->status_tbl.' s', 'lt.status = s.id','left')
	...
	function getUnreadTaskCount($userid){
		...
		$this->db->join($this->idea_tbl." ct", "t.createtask_id=ct.id");
