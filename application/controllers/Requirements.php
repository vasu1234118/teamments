...
class Requirements extends CI_Controller {
	...
	public $pagename='requirements';
	public $assigned_page='assignedtasks';
	...
	public $attachments_table='requirements_attachments';
	public $limit=30;
	public $add_title='Requirements';
	...
	function __construct(){
		...
		$this->data['assigned_page']=$this->assigned_page;
	...
	public function index()
	{
		...
		// exit;
		$this->data['records'] =$this->requirement->allRequirements();
	...
	public function add_new(){
		if($this->input->post()){
			...
			if($task_id){
				...
				$count = $this->common->getSelectedRowCount('*',$this->table,array('project_id'=>$record_data['project_id']));
				$uniq_id = 'PJ'.$record_data['project_id'].requirementId($count);
				//dd($uniq_id);
				$res = $this->common->updateTable($this->table,array('id'=>$task_id),array('uniq_id'=>$uniq_id));
				// var_dump($res);
				// exit;
				// Attachment Upload
				if(isset($_FILES['attachments'])) {
					...
					foreach($file_return as $fileloop){
						...
						$record_data=array('user_id'=>$this->data['ADMIN_ID'],'requirement_id'=>$task_id,'file_name'=>$fname);
				...
				for($i=0;$i<count($assigned_to);$i++){
					...
					$this->common->saveTable($this->requirements_table,$task_data);
		...
		// Getting Customers and users data form users table
		$this->data['all_users']=$this->common->getAllRecords('id,display_name,role', $this->users_table, '', array('id','DESC'), array($this->limit,0));
	...
	public function edit($ctask_md5_id='',$ctask_attach_md5_id=''){
		...
		$this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('user_id'=>$this->data['ADMIN_ID'],'md5(requirement_id)'=>$ctask_md5_id), array('id','DESC'), array($this->limit,0));

		$this->data['project']=$this->common->getAllRecords('id,name', $this->project_table, '', array('sort_order','DESC'), array($this->limit,0));
		
		$this->data['customers']=$this->common->getAllRecords('id,name', $this->project_table, '', array('id','DESC'), array($this->limit,0));
		$this->data['all_users']=$this->common->getAllRecords('id,display_name,role', $this->users_table, '', array('id','DESC'), array($this->limit,0));
        ...
        $this->data['priority']=$this->common->getAllRecords('id,title', $this->priority_table, '', array('order','ASC'), array($this->limit,0));
        $this->data['status']=$this->common->getAllRecords('id,title', $this->status_table, '', array('order','ASC'), array($this->limit,0));
		// $this->data['assigned_users']=$this->requirement->allAssignedUsers($this->data['ADMIN_ROLE_ID'],$this->data['ADMIN_ID']);
		// $this->data['allTasks'] =$this->requirement->allAssignedTasks($this->data['ADMIN_ID']);
