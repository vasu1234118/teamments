...
class Ideas extends CI_Controller {
	...
	public $attachments_table='ideas_attachments';
	public $limit=30;
	public $add_title='Idea';
	public $title='Ideas';
	...
	public function index()
	{
		...
		// exit;
		$this->data['records'] =$this->idea->allIdeas();
		$this->data['status']=$this->common->getAllRecords('id,title', $this->status_table, array('category_id'=>2), array('order','ASC'), array($this->limit,0));
		...
		if($this->input->post()){
			...
			if($task_id){
				...
				if(isset($_FILES['attachments'])) {
					...
					$file_return=$this->idea->upload_files($path, $title, $files);

					// Insert tm_createtasks_attachments
					foreach($file_return as $fileloop){
						if(isset($fileloop['file_name'])){
						$fname=$fileloop['file_name'];
						$record_data=array('user_id'=>$this->data['ADMIN_ID'],'idea_id'=>$task_id,'file_name'=>$fname);
				...
				$this->common->saveTable($this->ideas_table,$task_data);
	...
	public function edit($ctask_md5_id='',$ctask_attach_md5_id=''){
		...
		if($this->input->post()){
			...
			if($_FILES['attachments']['name'][0]) {
				...
				foreach($file_return as $fileloop){
					if(isset($fileloop['file_name'])){
					$fname=$fileloop['file_name'];
					$record_data1=array('user_id'=>$this->data['ADMIN_ID'],'idea_id'=>$this->data['record_info']->id,'file_name'=>$fname);
			...
			// for($i=0;$i<count($assigned_to);$i++){
			// 	$task_count=$this->common->get_table_count($this->ideas_table,array('user_id'=>$this->data['ADMIN_ID'],'md5(createtask_id)'=>$ctask_md5_id));
			// 	if($task_count==0){
		...
		$this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('user_id'=>$this->data['ADMIN_ID'],'md5(idea_id)'=>$ctask_md5_id), array('id','DESC'), array($this->limit,0));

		$this->data['complexity']=$this->common->getAllRecords('id,title', $this->complexity_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['priority']=$this->common->getAllRecords('id,title', $this->priority_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['assigned_users']=$this->idea->allAssignedUsers($this->data['ADMIN_ROLE_ID'],$this->data['ADMIN_ID']);
		$this->data['project']=$this->common->getAllRecords('id,name', $this->project_table, '', array('sort_order','DESC'), array($this->limit,0));
		$this->data['status']=$this->common->getAllRecords('id,title', $this->status_table, array('category_id'=>2), array('order','ASC'), array($this->limit,0));
		// $this->data['allTasks'] =$this->idea->allAssignedTasks($this->data['ADMIN_ID']);
