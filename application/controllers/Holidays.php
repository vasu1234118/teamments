...
class Holidays extends CI_Controller {
	...
	public $attachments_table='holiday_attachments';
	public $limit=30;
	public $add_title='Holiday';
	...
	public function index()
	{
		...
		// exit;
		$this->data['records'] =$this->holiday->allHolidays();
		...
		if($this->input->post()){
			...
			if($task_id){
				...
				if(isset($_FILES['attachments'])) {
					...
					$file_return=$this->holiday->upload_files($path, $title, $files);

					// Insert tm_createtasks_attachments
					foreach($file_return as $fileloop){
						if(isset($fileloop['file_name'])){
						$fname=$fileloop['file_name'];
						$record_data=array('user_id'=>$this->data['ADMIN_ID'],'holiday_id'=>$task_id,'file_name'=>$fname);
				...
				$this->common->saveTable($this->holidays_table,$task_data);
	...
	public function edit($ctask_md5_id='',$ctask_attach_md5_id=''){
		...
		if($this->input->post()){
            ...
            if(isset($_FILES['attachments'])) {
                if($_FILES['attachments']['name'][0]) {
                    ...
                    foreach($file_return as $fileloop){
                        if(isset($fileloop['file_name'])){
                        $fname=$fileloop['file_name'];
                        $record_data1=array('user_id'=>$this->data['ADMIN_ID'],'holiday_id'=>$this->data['record_info']->id,'file_name'=>$fname);
			...
			// for($i=0;$i<count($assigned_to);$i++){
			// 	$task_count=$this->common->get_table_count($this->holidays_table,array('user_id'=>$this->data['ADMIN_ID'],'md5(createtask_id)'=>$ctask_md5_id));
			// 	if($task_count==0){
		...
		// $this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('user_id'=>$this->data['ADMIN_ID'],'md5(holiday_id)'=>$ctask_md5_id), array('id','DESC'), array($this->limit,0));

		$this->data['complexity']=$this->common->getAllRecords('id,title', $this->complexity_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['priority']=$this->common->getAllRecords('id,title', $this->priority_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['assigned_users']=$this->holiday->allAssignedUsers($this->data['ADMIN_ROLE_ID'],$this->data['ADMIN_ID']);
		$this->data['project']=$this->common->getAllRecords('id,name', $this->project_table, '', array('sort_order','DESC'), array($this->limit,0));
		// $this->data['allTasks'] =$this->holiday->allAssignedTasks($this->data['ADMIN_ID']);
		$this->load->view($this->pagename.'/edit',$this->data);
	}

	function show($ctask_md5_id){

		//$this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('user_id='.$this->data['ADMIN_ID'].' AND md5(holiday_id)='.$ctask_md5_id), array('id','DESC'), array($this->limit,0));
