...
class Projects extends CI_Controller {
	...
	public $attachments_table='project_attachments';
	public $limit=30;
	public $add_title='Projects';
	...
	public function index()
	{
		...
		// exit;
		//dd($this->data);
		$this->data['records'] =$this->project->allProjects();
		...
		if($this->input->post()){
			...
			if($task_id){
				...
				if(isset($_FILES['attachments'])) {
					...
					$file_return=$this->project->upload_files($path, $title, $files);

					// Insert tm_createtasks_attachments
					foreach($file_return as $fileloop){
						if(isset($fileloop['file_name'])){
						$fname=$fileloop['file_name'];
						$record_data=array('user_id'=>$this->data['ADMIN_ID'],'project_id'=>$task_id,'file_name'=>$fname);
				...
				// $this->common->saveTable($this->table,$task_data);
	...
	public function edit($ctask_md5_id='',$ctask_attach_md5_id=''){
		...
		if($this->input->post()){
			...
			if($_FILES['attachments']['name'][0]) {
				...
				foreach($file_return as $fileloop){
					if(isset($fileloop['file_name'])){
					$fname=$fileloop['file_name'];
					$record_data1=array('user_id'=>$this->data['ADMIN_ID'],'project_id'=>$this->data['record_info']->id,'file_name'=>$fname);
			...
			// for($i=0;$i<count($assigned_to);$i++){
			// 	$task_count=$this->common->get_table_count($this->projects_table,array('user_id'=>$this->data['ADMIN_ID'],'md5(createtask_id)'=>$ctask_md5_id));
			// 	if($task_count==0){
			// 	$task_data=array('createtask_id'=>$this->data['record_info']->id,'user_id'=>$assigned_to[$i]);
			// 	$this->common->saveTable($this->projects_table,$task_data);
			// 	}
			// }
		...
		$this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('md5(project_id)'=>$ctask_md5_id), array('id','DESC'), array($this->limit,0));

		$this->data['complexity']=$this->common->getAllRecords('id,title', $this->complexity_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['priority']=$this->common->getAllRecords('id,title', $this->priority_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['assigned_users']=$this->project->allAssignedUsers($this->data['ADMIN_ROLE_ID'],$this->data['ADMIN_ID']);
		// $this->data['allTasks'] =$this->project->allAssignedTasks($this->data['ADMIN_ID']);
		$this->load->view($this->pagename.'/edit',$this->data);
	}

	function show($ctask_md5_id){
		$this->data['record_info'] =$this->project->getTaskDetails($ctask_md5_id);
		...
		$this->data['tasks_info']=$this->project->getAllAssignedTasks($this->data['record_info']->id);
		//print_r($this->data['tasks_info']);
		$this->data['unread_tasks']=$this->project->getUnreadTaskCount($this->data['ADMIN_ID']);
