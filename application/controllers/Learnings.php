...
class Learnings extends CI_Controller {
	...
	public $attachments_table='learning_attachments';
	public $limit=30;
	public $add_title='Learnings';
	public $title='Learnings';
	...
	public function index()
	{
		...
		// exit;
		$this->data['records'] =$this->learning->allLearnings();
	...
	public function add_new(){
		if($this->input->post()){
			...
			if($task_id){
				...
				$this->common->saveTable($this->learnings_table,$task_data);
	...
	public function edit($ctask_md5_id='',$ctask_attach_md5_id=''){
		...
		if($this->input->post()){
			...
			if($_FILES['attachments']['name'][0]) {
				...
				foreach($file_return as $fileloop){
					if(isset($fileloop['file_name'])){
					$fname=$fileloop['file_name'];
					$record_data1=array('user_id'=>$this->data['ADMIN_ID'],'learning_id'=>$this->data['record_info']->id,'file_name'=>$fname);
			...
			// for($i=0;$i<count($assigned_to);$i++){
			// 	$task_count=$this->common->get_table_count($this->learnings_table,array('user_id'=>$this->data['ADMIN_ID'],'md5(createtask_id)'=>$ctask_md5_id));
			// 	if($task_count==0){
		...
		$this->data['complexity']=$this->common->getAllRecords('id,title', $this->complexity_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['priority']=$this->common->getAllRecords('id,title', $this->priority_table, '', array('order','ASC'), array($this->limit,0));
		$this->data['assigned_users']=$this->learning->allAssignedUsers($this->data['ADMIN_ROLE_ID'],$this->data['ADMIN_ID']);
		$this->data['project']=$this->common->getAllRecords('id,name', $this->project_table, '', array('sort_order','DESC'), array($this->limit,0));
		// $this->data['allTasks'] =$this->learning->allAssignedTasks($this->data['ADMIN_ID']);
		$this->load->view($this->pagename.'/edit',$this->data);
	}

	function show($ctask_md5_id){

		$this->data['attachments']=$this->common->getAllRecords('id,file_name', $this->attachments_table, array('user_id='.$this->data['ADMIN_ID'].' AND md5(learning_id)='.$ctask_md5_id), array('id','DESC'), array($this->limit,0));
		...
		$this->data['record_info'] =$this->common->getSelectedRecordDetails('*', $this->table, array('md5(id)'=>$this->uri->segment(3)));
		//print_R($this->data['record_info']);die;
		$record_info_id=$this->data['record_info']->id;
