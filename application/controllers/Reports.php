...
class Reports extends CI_Controller
{
	...
	public $limit = 30;
	public $add_title = 'assignedtasks';
...
$project_id=null;
 if(isset($_POST['submit'])){
  $project_id=$this->input->post('project_id');
		...
		if($project_id){
			$this->data['records'] = $this->task->projectwise($this->data['ADMIN_ROLE_ID'], $this->data['ADMIN_ID'], $project_id);
		}
		else {
			$this->data['records']=[];
		}
		...
		if (isset($_GET['req_id']) && $_GET['req_id'] != '')
			$this->data['records'] = $this->task->projectwise($this->data['ADMIN_ID'], $_GET['req_id'],$project_id);
		else
			$this->data['records'] = $this->task->projectwise($this->data['ADMIN_ID'],'',$project_id);
	 }
		$this->load->view($this->pagename . '/projectwise', $this->data);
...
public function employewise()
	{
 	 ...
 	 $emp_id=$this->input->post('emp_id');
		...
		if (isset($_GET['req_id']) && $_GET['req_id'] != '' )
			$this->data['records'] = $this->task->employewise($this->data['ADMIN_ID'], $_GET['req_id'], $emp_id);
		else
			$this->data['records'] = $this->task->employewise($this->data['ADMIN_ID'],'', $emp_id);
	
		$this->load->view($this->pagename . '/employewise', $this->data);
	}
	...
	public function datewise()
	{
		 ...
		 if(isset($_POST['submit'])){
 	 $new_date=$this->input->post('started_on');
     $last_date=$this->input->post('last_date');
      $emp_id= $this->input->post('emp_id');
     $pr_id=$this->input->post('pr_id');

		if (isset($_GET['req_id']) && $_GET['req_id'] != '')
			$this->data['records'] = $this->task->datewise($this->data['ADMIN_ID'], $_GET['req_id'],$new_date, $last_date,$emp_id,$pr_id);
		else
			$this->data['records'] = $this->task->datewise($this->data['ADMIN_ID'],'', $new_date, $last_date,$emp_id,$pr_id);
	 }
		$this->load->view($this->pagename . '/datewise2', $this->data);
	}
	...
	public function empproject()
	{
     ...
     $pr_id=$this->input->post('pr_id');
		...
		$this->load->view($this->pagename . '/empproject', $this->data);
	}
}
